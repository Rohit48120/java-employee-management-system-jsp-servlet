	...
	private void applyLeave(HttpServletRequest request,HttpServletResponse response) throws SQLException,IOException {
		...
		String leaveFrom=request.getParameter("leaveFrom");
		 ...
		 if(message.equals("successfully applied... wait for review")) {
			 ...
			 response.getWriter().println("<html><body><b><script>alert(\"Leave requested successfully\")</script></b></body></html>");
		 }
			
		 else {
			 response.getWriter().println("<html><body><b><script>alert(\"Error adding leave, try again....\")</script></b></body></html>");
		}
...
private void updateProjectCompletionStatus(HttpServletRequest request,HttpServletResponse response) throws SQLException,IOException {
		...
		String status=request.getParameter("status");
		 ...
		 if(message.equals("status updated successfully..")) {
			 ...
			 response.getWriter().println("<html><body><b><script>alert(\"Project completion status updated\")</script></b></body></html>");
		 }
			
		 else {
			 response.getWriter().println("<html><body><b><script>alert(\"Failed to updated project status, try again\")</script></b></body></html>");
		}
	...
	private void signupEmployee(HttpServletRequest request,HttpServletResponse response) throws SQLException,IOException {
		 ...
		 if(str.equals("sign up successful wait for verification")) {
			 ...
			 response.getWriter().println("<html><body><b><script>alert(\"New employee registration successful, wait for verification\")</script></b></body></html>");
		 }
			
		 else {
			 response.getWriter().println("<html><body><b><script>alert(\"You may have provided wrong input..try again\")</script></b></body></html>");
		}
	...
	private void login(HttpServletRequest request,HttpServletResponse response) throws SQLException,IOException {
		...
		try {
		 ...
		 if(messasgeString.equals("LOGIN SUCCESSFUL!")) {
			 ...
			 response.getWriter().println("<html><body><b><script>alert(\"Incorrect username or password entered\")</script></b></body></html>");
		}
	}	
...
//        private void updateDetails(HttpServletRequest request, HttpServletResponse response) throws SQLException, IOException {
//    HttpSession session = request.getSession();
//    String username = (String) session.getAttribute("empusername");
//
//    try {
//        // Retrieve employee details from the database
//        Employee employee = dao.employeeDetails(username);
//        if (employee != null) {
//            // Pass employee details to the updatedetails.jsp for pre-filling the form
...
//            response.sendRedirect("error.jsp?message=" + URLEncoder.encode("Employee details not found", "UTF-8"));
//        }
//    } catch (EmployeeException | ServletException e) {
//        e.printStackTrace();
//        response.sendRedirect("error.jsp?message=" + URLEncoder.encode("Error retrieving employee details", "UTF-8"));
//    }
//}
